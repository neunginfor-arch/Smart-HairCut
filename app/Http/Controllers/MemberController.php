...
class MemberController extends Controller {
 ...
 public function dashboard(Request $request): View { $member=$request->attributes->get('member'); $upcoming=Booking::with(['branch','employee','service'])->where('member_id',$member->id)->whereIn('status',['pending','confirmed'])->whereDate('booking_date','>=',today())->orderBy('booking_date')->orderBy('start_time')->first(); return view('member.dashboard',['member'=>$member,'upcoming'=>$upcoming,'points'=>PointHistory::where('member_id',$member->id)->sum('points'),'couponCount'=>Coupon::where('is_active',true)->whereDate('valid_from','<=',today())->whereDate('valid_until','>=',today())->count(),'branches'=>Branch::where('is_active',true)->get(),'employees'=>Employee::where('is_active',true)->get(),'services'=>Service::where('is_active',true)->get()]); }
